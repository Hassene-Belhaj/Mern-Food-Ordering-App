  ...
  return (
    ...
    <Section $bg='#000' $color='#fff'>
        <Container $display='flex'  $width='95%' $LG_width='80%' $fd='row' $LG_fd='row' $MD_fd='column' $SM_fd='column' $XS_fd='column' $margin='auto' $padding='2rem 0 0 0' $fw='wrap'  >
            ...
            <Div $display='flex' $width='100%' $jc='start' $gap='1rem'>
                <Button   $fw='600' $width='9rem' $height='3rem' $outline='none' $border='none' $color='#000' $bg='#10b981' $br='25px' $opacity='0.9' $transition='all ease-in-out 0.3s '>Order Now</Button>
                <Navlink to='/menu'>
                  <Button  $fw='600' $width='9rem' $height='3rem' $color='#10b981' $outline='none' $border='2px solid #10b981' $bg='transparent' $br='25px' $opacity='0.9' $transition='all ease-in-out 0.3s '>See Menu</Button>
                </Navlink>
            </Div>    
            </Div>
